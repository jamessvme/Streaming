...
<div
	class={`inline-flex appearance-none items-center justify-center select-none relative whitespace-nowrap align-middle outline outline-transparent outline-2 outline-offset-2 leading-tight rounded-md font-semibold transition max-w-[40px] h-10 px-4 ${schemeF} ${
		disabled ? 'cursor-not-allowed opacity-40' : 'cursor-pointer'
	} ${variantF}`}
	on:click={() => handleClick()}
>
	<div class="max-w-4 h-4">
		<Fa {icon} style="color: currentColor" />
	</div>
</div>
