	...
	const handleCreate = async () => {
		...
		if ($LibraryCreateForm.dirty && $LibraryCreateForm.valid) {
			try {
				...
				const response = await axios.post(`${env.PUBLIC_FLOGRAM_API_URL}/libraries/`, {
	...
	<div class="flex flex-col gap-5">
		...
		<div class="flex gap-5">
			...
			<div class="flex flex-col gap-1">
				...
				<Input
					...
					isInvalid={$LibraryCreateForm.hasError('libraryName.required')}
				/>
		...
		<div class="flex items-center gap-2">
			...
			<span class="text-sm text-gray-500"
				>You are creating a {isPublic ? 'public' : 'private'} library in your work space.</span
			>
		</div>
