<script lang="ts">
	...
	const checkUsername = () => {
		return async (username: string) => {
			...
			return {
				valid: users.find((user: any) => user.username === username) === undefined,
				name: 'already_taken'
			};
		};
	};
	...
	const handleSignup = async () => {
		...
		if ($signupForm.dirty && $signupForm.valid) {
			try {
				isLoading = true;
				const response = await axios.post(`${env.PUBLIC_FLOGRAM_API_URL}/auth/signup`, {
					email: $emailF.value,
				...
				toasts.add({
					...
					description:
						'You account is created successfully. Please log in with your account and enjoy with Flogram!',
...
<div class="dark:bg-gray-900 max-w-[1200px] mx-auto min-h-[100vh] flex">
	<!-- left side -->
	<div
			...
			<div class="dark:text-gray-100 flex flex-col gap-8 w-full">
				<div class="flex flex-col text-center gap-2">
					...
					<div class="dark:text-gray-500 text-gray-700 flex items-center justify-center">
						...
						<Link href="/app/auth/signin" class="dark:text-gray-100 ml-3">Login</Link>
					</div>
				</div>
				...
				<div class="flex flex-col gap-5">
					...
					<div class="flex flex-col gap-1">
						<span class="dark:text-gray-500 text-sm font-medium text-gray-700">Email</span>
						<Input
							type="text"
							placeholder="Enter your email"
							bind:value={$emailF.value}
							isInvalid={$signupForm.hasError('email.required')}
						/>
						{#if !$signupForm.valid}
					...
					<!-- username -->
					<div class="flex flex-col gap-1">
						<span class="dark:text-gray-500 text-sm font-medium text-gray-700">User name</span>
						<Input
							type="text"
							placeholder="Enter your username"
							bind:value={$usernameF.value}
							isInvalid={$signupForm.hasError('username.required')}
					...
					<div class="flex flex-col gap-1">
						...
						<Input
							...
							isInvalid={$signupForm.hasError('password.required')}
						/>
						{#if !$signupForm.valid}
					...
					<!-- confirm password -->
					<div class="flex flex-col gap-1">
						<span class="dark:text-gray-500 text-sm font-medium text-gray-700"
						...
						<Input
							...
							isInvalid={$signupForm.hasError('password2.required')}
						/>
						{#if !$signupForm.valid}
							{#if $signupForm.hasError('password.required')}
